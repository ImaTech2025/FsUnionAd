...
import com.ima.union.core.model.impl.FsUnionRewardedVideoAd;
 ...
 */
public class FsRewardedVideoAdManager {

    private static final String TAG = "FsRewardedVideoAdManager";

        public interface OnFsRewardedVideoAdLoadListener {
        void onAdLoaded(IFsUnionRewardedVideoAd ad);
        void onAdLoadError(int errorCode, String errorMsg);
    }

    public static void loadAd(Context context, AdRequestParams params,
                              OnFsRewardedVideoAdLoadListener loadListener) {
        String slotId = params.getSlotId();
        FsLogger.d("RewardedAdMgr", "▶ loadAd: slotId=" + slotId);
        ...
        if (mergedUnitConfig == null) {
            FsLogger.e("RewardedAdMgr", "loadAd: merged config is null, slotId=" + slotId);
            if (loadListener != null)
                loadListener.onAdLoadError(FsAdErrorCode.CONFIG_NULL, FsAdErrorCode.buildMsg("合并配置为空"));
            return;
        }

        mergedUnitConfig.setAdFormat(AdFormat.REWARDED_VIDEO);
                ...
                new AdStrategyCallback() {
                    ...
                    public void onAdLoaded(UnionAdResponse response) {
                        ...
                        AdAdapter adapter = sourceConfig != null
                                ...
                                : null;
                        if (adapter == null) {
                            FsLogger.e("RewardedAdMgr", "onAdLoaded: cannot resolve adapter for " + response.getSdkName());
                            if (loadListener != null)
                                loadListener.onAdLoadError(FsAdErrorCode.ADAPTER_RESOLVE_FAILED, FsAdErrorCode.buildMsg("无法解析适配器"));
                            return;
                        }
                        IFsUnionRewardedVideoAd ad = new FsUnionRewardedVideoAd(context, slotId, response, adapter);
                    ...
                    public void onNoFill(String reason) {
                        FsLogger.w("RewardedAdMgr", "loadAd: no fill, " + reason);
                        if (loadListener != null)
